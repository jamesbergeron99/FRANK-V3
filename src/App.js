...
const App = () => {
  ...
  const handleFrankResponse = async (textToProcess, isDeepDive = false) => {
    ...
    try {
      ...
      if (!responseText) {
        throw new Error("No response from Frank. Check API Key or Safety Filters.");
      }
    ...
    <div className="flex flex-col h-screen bg-[#faf9f6] text-[#2c2c2c] overflow-hidden text-sm">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        {activeTab === 'chat' ? (
          ...
          <div className="flex-1 overflow-y-auto p-16 bg-white max-w-4xl mx-auto w-full shadow-lg my-8 rounded-lg">
            <h2 className="text-3xl font-black uppercase mb-8 border-b-4 border-black pb-4">Executive Script Coverage</h2>
            {scriptData ? (
              <div className="space-y-8">
                <div className="flex items-center gap-4">
                  <span className="font-bold text-lg uppercase">Status:</span>
                  <span className={`px-4 py-2 rounded-full font-black text-white ${scriptData.grade === 'GREEN LIGHT' ? 'bg-green-600' : 'bg-red-600'}`}>
                    {scriptData.grade}
                  </span>
                </div>
              ...
              <div className="text-center py-20 italic text-stone-400">
                Frank hasn't read anything yet. Upload a script in the Lounge.
              </div>
            )}
          </div>
        ) : null}
