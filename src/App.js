...
const App = () => {
  ...
  const handleScriptUpload = async (file) => {
    ...
    try {
      ...
      // Automatically parse for Read-Through
      const lines = text.split('\n').filter(l => l.trim());
      const newCast = new Set();
      const parsed = lines.map(l => {
        const trimmed = l.trim();
        const isChar = trimmed === trimmed.toUpperCase() && trimmed.length > 2 && trimmed.length < 25;
        if (isChar) newCast.add(trimmed);
        return { type: isChar ? 'character' : 'dialogue', text: trimmed };
      });
      setParsedLines(parsed);
      setCast(Array.from(newCast));
  ...
  const handleFrankResponse = async (text) => {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ contents: [{ parts: [{ text: `You are Frank, Sunset Blvd script doctor. Give critical feedback: ${text}` }] }] })
      });
      const data = await res.json();
      const frankText = data.candidates?.[0]?.content?.parts?.[0]?.text || "Connection glitch.";
  ...
  const generatePoster = async () => {
    if (!lastScriptContent) return alert("Upload a script first, kid.");
    setIsGeneratingPoster(true);
    try {
      // 1. Get Visual Prompt from Gemini
      const promptUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`;
      const promptRes = await fetch(promptUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contents: [{ parts: [{ text: `Create a cinematic movie poster prompt for: ${lastScriptContent.slice(0, 2000)}` }] }] })
      });
      const promptData = await promptRes.json();
      const visualPrompt = promptData.candidates[0].content.parts[0].text;

      // 2. Call Imagen (Simulated for this block, replace with actual endpoint if needed)
      setPosterUrl(`https://via.placeholder.com/600x900.png?text=POSTER+GENERATED`);
          ...
          <div className="flex-1 flex flex-col bg-white overflow-hidden">
            <div className="bg-stone-900 text-white p-4 flex items-center justify-between px-8">
               <div className="flex items-center gap-4">
                  ...
                  <button onClick={() => setReadState('playing')} className="px-8 py-2 bg-white text-black rounded-full font-black uppercase text-xs">Play Read</button>
               </div>
            ...
            <div className="flex-1 flex overflow-hidden">
                  ...
                  <div className="space-y-4">
                     {cast.map(char => (
                        <div key={char} className="bg-white p-4 rounded-xl border">
                           ...
                           <select className="w-full bg-stone-50 border rounded p-2 text-xs text-black">
               ...
               <div className="w-2/3 p-16 overflow-y-auto font-serif text-lg leading-relaxed text-black">
                  {parsedLines.length > 0 ? parsedLines.map((line, idx) => (
                    <div key={idx} className={`p-2 rounded ${currentLineIndex === idx ? 'bg-stone-100 font-bold' : ''}`}>{line.text}</div>
                  )) : <div className="text-stone-300 italic">Upload a script in the Lounge or via the button above.</div>}
               </div>
            </div>
          </div>
        )}
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-20 bg-stone-50">
            {posterUrl ? (
              ...
              <div className="text-center">
                <button onClick={generatePoster} disabled={isGeneratingPoster} className="px-10 py-5 bg-black text-white rounded-full font-black uppercase tracking-widest">
                  {isGeneratingPoster ? "Painting..." : "Generate Movie Poster"}
                </button>
                <p className="mt-4 text-stone-400 text-xs uppercase font-bold tracking-widest">Requires a script in the system</p>
              </div>
            )}
          </div>
        )}
