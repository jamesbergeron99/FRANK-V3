...
const App = () => {
  ...
  const handleScriptUpload = async (file) => {
    ...
    } catch (e) { setErrorMessage("PDF Engine failed."); }
    finally { setIsProcessing(false); }
  };

  // --- RESTORED DICTATION ENGINE ---
  ...
  const handleFrankResponse = async (text) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'user', content: "[Input Processed]" }, { role: 'assistant', content: responseText }]);
    } catch (e) { setErrorMessage("API handshake failed."); } finally { setIsProcessing(false); }
  };
