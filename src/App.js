...
const App = () => {
  ...
  // --- BRAIN LOGIC (FIXED FOR V1 STABLE) ---
  const handleFrankResponse = async (text) => {
    ...
    try {
      // Changed to v1 endpoint which is the most compatible with stable API keys
      ...
      if (data.error) {
        setMessages(prev => [...prev, { role: 'assistant', content: `Studio Error: ${data.error.message}` }]);
        return;
      }
      ...
      if (frankText) {
        ...
        setMessages(prev => [...prev, { role: 'assistant', content: "The connection is dead air. Try again?" }]);
      }
    } catch (e) {
      setMessages(prev => [...prev, { role: 'assistant', content: "Lost the link to the lot. Check your keys." }]);
    } finally {
        ...
        {activeTab === 'chat' ? (
          <>
            <div className="flex-1 overflow-y-auto p-10 space-y-10">
              ...
              {isProcessing && <div className="p-10 animate-pulse text-stone-400 font-bold italic text-center">Frank is marking up the pages...</div>}
              <div ref={scrollRef} />
            </div>
          ...
          <div className="flex-1 flex flex-col bg-white">
            ...
            <div className="flex-1 flex overflow-hidden">
                  ...
                  <div className="space-y-4">
                     {['Narrator', ...cast].map(char => (
                        <div key={char} className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm">
                           ...
                           <select className="w-full bg-stone-50 border rounded-lg p-2.5 text-xs outline-none text-black font-bold">
               ...
               <div className="w-2/3 p-20 font-serif text-xl text-stone-300">Upload a script in the Lounge to begin.</div>
            </div>
          </div>
        )}
      </main>
