...
const App = () => {
  ...
  // --- RESTORED API CALL (EARLY RENDITIONS STYLE) ---
  const handleFrankResponse = async (text) => {
    ...
    try {
      ...
      if (data.candidates && data.candidates[0].content) {
        setMessages([...currentMsgs, { role: 'assistant', content: data.candidates[0].content.parts[0].text }]);
      }
    } catch (e) {
      setMessages([...currentMsgs, { role: 'assistant', content: "Handshake Failed. Checking Render configuration..." }]);
    } finally {
