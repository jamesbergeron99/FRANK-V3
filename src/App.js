...
const App = () => {
  ...
  useEffect(() => {
    signInAnonymously(auth).catch(() => setErrorMessage("Authentication Failed. Check Keys."));
  ...
  const handleFrankResponse = async (text) => {
    ...
    } catch (e) { setErrorMessage("Analysis Stalled. Check Render Keys."); } finally { setIsProcessing(false); }
  };
