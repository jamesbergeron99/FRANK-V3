...
const App = () => {
  ...
  const fetchAudioChunk = async (text, customVoiceId = VOICE_ID) => {
    try {
      ...
      // Logic fix: Ensure customVoiceId is passed correctly
    ...
    const res = await fetchWithRetry(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts: [{ text }] }], generationConfig: { responseModalities: ["AUDIO"], speechConfig: { voiceConfig: { voiceName: "Zephyr" } } } } }) });
  ...
  const handleFrankResponse = async (textToProcess, isDeepDive = false) => {
    ...
    try {
      // Fix: Stable Gemini model URL and clean mapping for handshake stability
      const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent?key=${apiKey}`;
    ...
    } catch (e) { setErrorMessage("Analysis failed. Connection stalled."); } finally { setIsProcessing(false); setIsDeepDiving(false); }
  };
            ...
            <div className="flex-1 flex overflow-hidden">
               ...
               <div className="w-2/3 p-16 overflow-y-auto font-serif text-lg leading-relaxed bg-[#fdfcfb]">
                  {parsedLines.map((line, idx) => (
                    <div key={idx} ref={currentLineIndex === idx ? activeLineRef : null} className={`p-2 rounded mb-2 ${currentLineIndex === idx && readState === 'playing' ? 'bg-stone-900 text-white' : ''}`} style={{ marginLeft: line.type === 'character' ? '20%' : (line.type === 'dialogue' ? '15%' : '0'), textAlign: line.type === 'transition' ? 'right' : 'left' }}>{line.text}</div>
                  ))}
               </div>
            </div>
          </div>
