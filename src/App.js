...
const extractTextFromPDF = async (file) => {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    const items = content.items.slice().sort((a, b) => b.transform[5] - a.transform[5] || a.transform[4] - b.transform[4]);
    let currentLine = "";
          ...
          <>
            ...
            <div className="bg-white border-t p-5 shrink-0 shadow-lg">
              <div className="flex items-center gap-4 max-w-5xl mx-auto w-full">
                ...
                <button onClick={() => handleFrankResponse(null, true)} className="bg-red-600 text-white px-6 py-3 rounded-full font-black uppercase text-[10px] tracking-widest shadow-lg">Surgery</button>
              </div>
            </div>
          </>
