...
const App = () => {
  ...
  const handleFrankResponse = async (text) => {
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ 
          contents: [{ parts: [{ text: `You are Frank, a blunt Sunset Blvd script doctor. Give critical feedback on this: ${text}` }] }] 
        })
      });
  ...
  const handleScriptUpload = async (file) => {
    ...
    try {
      ...
      handleFrankResponse(`[Script Uploaded] Analyze this script: ${text.slice(0, 5000)}`);
        ...
        {activeTab === 'chat' ? (
          <>
            <div className="flex-1 overflow-y-auto p-10 space-y-10">
              ...
              {isProcessing && <div className="p-10 animate-pulse text-stone-400 font-bold italic text-center">Frank is considering...</div>}
              <div ref={scrollRef} />
            </div>
            <div className="bg-white border-t p-5 shrink-0 shadow-lg">
              <div className="flex items-center gap-4 max-w-5xl mx-auto w-full">
                <input value={inputText} onChange={e => setInputText(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleFrankResponse(inputText)} placeholder="Defend your arc..." className="flex-1 px-6 py-4 bg-stone-50 rounded-xl outline-none text-black placeholder:text-stone-300" />
                <button onClick={() => handleFrankResponse(inputText)} className="w-12 h-12 bg-stone-800 text-white rounded-full flex items-center justify-center shadow-md hover:bg-black transition-all"><Send size={18} /></button>
                <label className="w-12 h-12 bg-stone-50 rounded-full flex items-center justify-center cursor-pointer text-stone-400 hover:text-black hover:bg-stone-100 transition-all"><FileUp size={20}/><input type="file" className="hidden" accept=".pdf" onChange={(e) => handleScriptUpload(e.target.files[0])}/></label>
              </div>
            </div>
          </>
        ) : (
          <div className="flex-1 flex flex-col bg-white">
            <div className="bg-stone-900 text-white p-3 flex items-center justify-between px-8">
               <h2 className="text-[10px] font-bold uppercase tracking-widest">Table Read Mode</h2>
            ...
            <div className="flex-1 flex overflow-hidden">
               <div className="w-1/3 bg-[#faf9f6] border-r p-10 overflow-y-auto">
                  <h2 className="text-xl font-black uppercase mb-8 flex items-center gap-3 text-black"><Users size={20}/> Voice Casting</h2>
                  <div className="space-y-4">
                     {['Narrator', ...cast].map(char => (
                        <div key={char} className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm">
                           ...
                           <select value={voiceAssignments[char] || ''} onChange={e => setVoiceAssignments(v => ({...v, [char]: e.target.value}))} className="w-full bg-stone-50 border rounded-lg p-2.5 text-xs outline-none text-black font-bold">
