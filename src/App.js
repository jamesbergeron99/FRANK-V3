...
const App = () => {
  ...
  const parseScript = (text) => {
    ...
    const parsed = lines.map(line => {
      ...
      if (isChar) characters.add(trimmed);
      return { type: isChar ? 'character' : 'dialogue', text: trimmed, character: isChar ? trimmed : null };
    });
  ...
  const handleFrankResponse = async (text) => {
    ...
    try {
      ...
      const res = await fetchWithRetry(url, {
        ...
        body: JSON.stringify({ contents: [{ parts: [{ text: `You are Frank, a blunt script executive. Analysis of: ${text}` }] }] })
      });
    ...
    <div className="flex flex-col h-screen bg-[#faf9f6] text-[#2c2c2c] overflow-hidden text-sm">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        {activeTab === 'chat' ? (
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-10">
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[75%] p-6 rounded-2xl shadow-sm ${m.role === 'user' ? 'bg-stone-800 text-white' : 'bg-white border shadow-sm text-black'}`}>{m.content}</div>
                </div>
              ))}
          ...
          <div className="flex-1 flex flex-col bg-white overflow-hidden text-black font-sans">
            ...
            <div className="flex-1 flex overflow-hidden">
               <div className="w-1/3 bg-[#faf9f6] border-r p-10 overflow-y-auto">
                  ...
                  <div className="space-y-4">
                     <div className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm">
                        <div className="font-bold text-[10px] uppercase mb-2">Narrator</div>
               ...
               <div className="w-2/3 p-16 overflow-y-auto font-serif text-lg leading-relaxed text-stone-300 italic">
                  ...
                  )) : "Upload a script to cast characters."}
               </div>
            </div>
          </div>
        )}
